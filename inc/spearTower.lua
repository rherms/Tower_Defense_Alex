	...
	if t.level < 5 then 
		if money >= cost then 
			...
			elseif t.level == 5 then
				t.piercingLevel = t.piercingLevel + 2
			end
...
local function showTowerInfo(event)
	if not displayingMessage then 
		...
		message = message .. "\nPiercing Level: " .. t.piercingLevel
		if t.buffed then
...
function newSpearTower:new(params)
    local spearTower = {}
	
	--create a shape
	--spearTower = display.newImage("img/spearTower.png", params.x, params.y)
	spearTower = widget.newButton( {x = params.x, y = params.y, defaultFile = "img/spearTower.png", onRelease = showTowerOptions} )
	spearTower.first = true
	spearTower.pressed = false
	globalSceneGroup:insert(spearTower)

	spearTower.x = params.x
	spearTower.y = params.y
	spearTower.damage = 60
	
	spearTower.shootRate = 1.5 -- shots per second
	spearTower.shootSleep = 100
	spearTower.shootSleepMax = 30 / spearTower.shootRate; 
	
	spearTower.shootRange = 120
	spearTower.shootRangeObject = display.newCircle( spearTower.x, spearTower.y, spearTower.shootRange )
	globalSceneGroup:insert(spearTower.shootRangeObject)
	spearTower.shootRangeObject:setFillColor(1, 0, 0, 20/255)
	spearTower.shootRangeObject.isVisible = false
	
	spearTower.target = nil
	spearTower.cost = 200
	spearTower.level = 1
	spearTower.index = params.index
	spearTower.piercingLevel = 1 --initially can hit 2 enemies total
	spearTower.buffed = false
	table.insert(towers, spearTower)

	spearTower.upgrades = {"Range + 30", "Damage + 10", "Shoot Rate + 0.3", "Piercing + 2"}

	function spearTower:enterFrame(e)
		if not paused then
	...
	function spearTower:enterFrameAI()
	...
	function spearTower:selectTarget()
		if ( #enemies ) then
		
			--try to find enemy, who is nearest to self
			local enemyKey = nil; --key of enemy who has a minimal distance to current spearTower
	...
	function spearTower:checkTarget()
	...
	function spearTower:enemyInRange( enemy )
		local distance = utils.getDistance( self.x, self.y, enemy.x + enemy.width, enemy.y + enemy.height)
		return distance <= self.shootRange
	end
	
	--check available of shooting
	function spearTower:canIShoot()
		return ( self.shootSleep > self.shootSleepMax )
	end
	
	--make a shot (create a bullet)
	function spearTower:shoot( target )
		if ( self:canIShoot() ) then
			Spear:new( { 
				damage = self.damage,
				piercingLevel = self.piercingLevel,
				x = self.x,
				y = self.y,
	...
	Runtime:addEventListener('enterFrame', spearTower)
	
	return spearTower
end

return newSpearTower